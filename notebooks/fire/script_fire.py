def fire_csv():
    ...
    data_fire = pd.read_csv('./data/Incendies.csv',
                      sep=';',
                      skiprows=3)
    ...
    data_fire['year_quarter'] = data_fire['year'].astype(str) + 'T' + data_fire['quarter'].astype(str)
    ...
    agg_data_fire = data_fire.groupby(['department', 'year', 'quarter', 'year_quarter']).agg({
    'area burned (m²)': 'sum',
    'area (m²)': 'first'
    }).reset_index()
    ...
    agg_data_fire['ratio'] = agg_data_fire['area burned (m²)'] / agg_data_fire['area (m²)']
    ...
    agg_data_fire[['department', 'year', 'quarter', 'year_quarter', 'ratio', 'danger_fire_level']],
    ...
    data_fire['fire_score'] = data_fire['danger_fire_level'].map(risk_mapping).astype(float)
    print('Data fire final shape:', data_fire.shape)

    fire_score_dept_quarter = data_fire.groupby(['department', 'year_quarter'])['fire_score'].mean() \
        .reset_index()\
